...
const initialState = {
  ...
  // Ad listing grid/list toggle. Lives here rather than in the listing itself:
  // the toolbar sits outside the listing's Suspense boundary and the cards
  // inside it, separated by a server component that no prop can cross.
  AdsView: "grid",
};
...
export const globalStateSlice = createSlice({
  ...
  reducers: {
    ...
    setAdsView: (state, action) => {
      state.AdsView = action.payload;
    },
  },
});
...
export const getAdsView = createSelector(
  (state) => state.GlobalState,
  (GlobalState) => GlobalState.AdsView
);
