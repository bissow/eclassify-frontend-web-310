...
import { getFollowersApi, getFollowingApi, unfollowUserApi } from "@/lib/api";
        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="p-0 [&>button]:hidden">
                ...
                >
                    ...
                    <TabsContent
                        value="followers"
                        className="max-h-112.5 overflow-y-auto py-2"
                    >
                        {followersLoading ? (
                            <UserListSkeleton />
                        ) : followers.length === 0 ? (
                            <NoData title={t("noFollowersFound")} />
                        ) : (
                            <>
                                {followers.map((user) => (
                    ...
                    >
                        {followingLoading && following.length === 0 ? (
                            ...
                            <NoData title={t("notFollowingAnyoneYet")} />
                        ) : (
                            <>
                                {following.map((user) => (
                                    <div key={user.id} className="flex items-center justify-between py-4">
