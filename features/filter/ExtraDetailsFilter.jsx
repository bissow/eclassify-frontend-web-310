...
const ExtraDetailsFilter = ({ customFields, newSearchParams }) => {
  const { t } = useTranslation();

  const basePath = useAdsBasePath();
  const { navigate } = useNavigate();
  const { initialExtraDetails } = useAdsFilters();
  ...
  const handleCheckboxChange = (id, value, checked) => {
    setExtraDetails((prev) => {
      const raw = prev[id];
    const existing = Array.isArray(raw) ? raw : raw ? String(raw).split(",").filter(Boolean) : [];
