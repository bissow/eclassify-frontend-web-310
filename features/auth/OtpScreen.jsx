...
import { getOtpApi, userSignUpApi, verifyOtpApi } from "@/lib/api";
import { loadUpdateData } from "@/store/slices/authSlice";
import { useSelector } from "react-redux";
import { Fcmtoken, getOtpServiceProvider } from "@/store/slices/settingSlice";
...
const OtpScreen = ({
  ...
  const verifyOTPWithFirebase = async () => {
    try {
      ...
      if (isRegister) {
        payload.name = `user_${crypto.randomUUID().slice(0, 8)}`;
    ...
    <form className="flex flex-col gap-6" onSubmit={verifyOTP}>
      <div className="labelInputCont">
        ...
        <Input
          ...
          ref={otpInputRef}
          className={cn(otpError && "border-destructive focus-visible:ring-destructive")}
        />
        {otpError && (
