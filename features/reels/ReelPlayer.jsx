...
import { chatListApi, itemOfferApi, getMyReelsApi, getReelsApi, manageReelLikeApi, followUserApi, unfollowUserApi } from "@/lib/api";
import { getIsLoggedIn, userSignUpData } from "@/store/slices/authSlice";
import { getCompanyName, getDefaultLanguageCode } from "@/store/slices/settingSlice";
...
const ActionButtons = ({ isMuted, onToggleMute, onLike, isLiked, likedCount, onChat, isStartingChat, isSelf, isLoggedIn, shareUrl, shareTitle, shareHeadline, companyName }) => {
...
const ReelPlayer = ({ initialId }) => {
  ...
  const reel = reels[currentIndex];
  const isSelf = userData?.id === reel?.item?.user?.id;
  const itemPrice = reel?.item?.formatted_price || reel?.item?.formatted_salary_range;
  const isHidePrice = !itemPrice
  const reelPath = `/${lang}/reel/${reel?.id}`;
  const shareUrl = `${process.env.NEXT_PUBLIC_WEB_URL}${reelPath}`;
  const shareTitle = `${reel?.item?.translation?.name || reel?.item?.name} | ${companyName}`;
  const shareHeadline = `🚀 Discover the perfect deal! Explore "${reel?.item?.translation?.name || reel?.item?.name}" from ${companyName} and grab it before it's gone. Shop now at`;
          ...
          {reels.length === 0 ? (
            ...
            >
              ...
              <div className="md:hidden absolute right-2 top-1/2 -translate-y-1/2 z-10">
                <ActionButtons isMuted={isMuted} onToggleMute={() => setIsMuted((m) => !m)} onLike={handleLike} isLiked={reel?.is_liked} likedCount={reel?.liked_count} onChat={handleChat} isStartingChat={isStartingChat} isSelf={isSelf} isLoggedIn={isLoggedIn} shareUrl={shareUrl} shareTitle={shareTitle} shareHeadline={shareHeadline} companyName={companyName} />
              </div>
                ...
                <div className="flex items-center gap-2">
                  <CustomLink href={isSelf ? "/profile" : `/seller/${reel?.item?.user?.id}`} className="flex items-center gap-2 min-w-0">
                    <UserAvatar
                      ...
                      initial={reel?.item?.user?.initial}
                      avatarColor={reel?.item?.user?.avatar_color}
