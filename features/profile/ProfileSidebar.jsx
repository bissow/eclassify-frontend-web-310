...
import DeleteAccountVerifyOtpModal from "@/features/auth/DeleteAccountVerifyOtpModal";
import {
...
import { BellIcon, BriefcaseIcon, ChatsIcon, CurrencyCircleDollarIcon, FireIcon, HeartIcon, QrCodeIcon, ReceiptIcon, ShoppingBagOpenIcon, SignOutIcon, StarIcon, TrashSimpleIcon, UserIcon } from "@phosphor-icons/react";
...
const ProfileSidebar = () => {
  ...
  const { signOut, revokeToken } = FirebaseData();
  ...
  const handleLogout = async () => {
    try {
      ...
      // Unregister this device's FCM token so it stops receiving push.
      const fcmToken = await revokeToken();
      await signOut();
      const res = await logoutApi.logoutApi({
        fcm_token: fcmToken || userData?.fcm_id || "",
      });
  ...
  return (
    <>
      <div className="flex flex-col gap-4 py-6 px-4 h-full">
        ...
        <CustomLink
          href="/seller-qr"
          className={`flex items-center gap-1 py-2 px-4 ${isActivePath("/seller-qr") ? "profileActiveTab" : ""
            }`}
        >
          <QrCodeIcon size={24} weight="bold" />
          <span>{t("sellerQrStandee") || "Store QR Standee"}</span>
        </CustomLink>
        <CustomLink
          href="/favorites"
