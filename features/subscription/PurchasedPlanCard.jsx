...
import { formatPriceAbbreviated, formatSubscriptionDate } from "@/lib/format";
import CustomImage from "@/components/common/CustomImage";
import { useState } from "react";
import { Progress } from "@/components/ui/progress";
import { CheckFatIcon, PlayCircleIcon } from "@phosphor-icons/react";
...
const PurchasedPlanCard = ({ pckg }) => {
  ...
  return (
    <div className="perspective-1000 h-full">
      ...
      >
        <div
          className={`h-full backface-hidden rounded-lg relative p-3 sm:p-6 shadow-xs border bg-white ${isFlipped ? "pointer-events-none" : ""}`}
        >
          {/* Card Header */}
        ...
        <div
          className={`absolute inset-0 rotate-y-180 backface-hidden rounded-lg p-4 sm:p-8 shadow-xs border bg-white flex flex-col ${!isFlipped ? "pointer-events-none" : ""}`}
        >
          ...
          <div className="flex flex-col gap-2 overflow-y-auto flex-1">
            {pckg?.categories_path?.map((category) => (
              <div key={category.id} className="flex items-center gap-3">
                <CheckFatIcon weight="fill" className="text-primary" />
