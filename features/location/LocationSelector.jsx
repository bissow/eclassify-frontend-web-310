...
import { ArrowLeftIcon, CaretRightIcon, CircleNotchIcon, GpsFixIcon, MagnifyingGlassIcon } from "@phosphor-icons/react";
import { useInView } from "react-intersection-observer";
import { getAreasApi, getCitiesApi, getCoutriesApi, getStatesApi } from "@/lib/api";
import {
  resetCityData,
  saveCity,
  saveKilometerRange,
...
import { getMinRange } from "@/store/slices/settingSlice";
...
const LocationSelector = ({ OnHide, selectedCity, setSelectedCity, setIsMapLocation, shouldSaveToRedux = true }) => {
  ...
  const handleSubmitLocation = () => {
    minLength > 0 ? saveKilometerRange(minLength) : saveKilometerRange(0);
    // Always navigate, even if already on "/" — without Redux, this is the
    // only way server-rendered sections (AllItemsSection etc.) pick up the
    // fresh location cookie and any still-client location-reactive component
    // gets a chance to re-run its effects.
    navigate("/");
  };
  ...
  const getTitle = () => {
    switch (currentView) {
      case "countries":
        return t("noCountriesFound");
      case "states":
        return t("noStatesFound");
      case "cities":
        return t("noCitiesFound");
      case "areas":
        return t("noAreasFound");
    }
  };
    ...
    <>
      ...
      {currentView === "countries" ? (
        ...
        <div className="flex items-center gap-2 border rounded-sm relative p-3 ltr:pl-9 rtl:pr-9">
          <MagnifyingGlassIcon className="size-4 shrink-0 absolute left-3 text-muted-foreground" />
          <input
            type="text"
...

      {
        IsBrowserSupported && (
          <div className="flex items-center gap-2 border rounded p-2 px-4">
            <GpsFixIcon size={20} weight="fill" className="text-primary size-5 shrink-0" />
            <button
