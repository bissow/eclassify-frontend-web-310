...
import { EnvelopeIcon, PhoneCallIcon, ShieldCheckIcon, StarIcon } from "@phosphor-icons/react";

// Seller comes from the server; this component owns it after mount only so
// follow/unfollow can update the counts without a refetch.
const SellerDetailCard = ({ initialSeller, ratingCount }) => {
  ...
  const memberSinceYear = seller?.created_at
    ...
    : "";
  const mobilePrefix = seller?.country_code ? `+${seller.country_code} ` : "";
  const currentUrl = `${process.env.NEXT_PUBLIC_WEB_URL}/${lang}/seller/${seller?.id}`;
          ...
          <div className="border-t p-4 flex flex-col gap-4">
            ...
            {seller?.mobile && (
              <div className="flex items-center gap-2">
                ...
                <Link
                  href={`tel:${mobilePrefix}${seller?.mobile}`}
                  className="break-all"
                >
                  {mobilePrefix}{seller?.mobile}
                </Link>
              </div>
            )}
          </div>
