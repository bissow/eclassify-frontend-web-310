...
export default async function sitemap(props) {
  ...
  try {
    const res = await fetch(
      `${apiUrl}${endpoint}get-item-slug?page=${page}`,
      { next: { revalidate: SITEMAP_REVALIDATE_SECONDS } }
    );
    if (!res.ok) return [];
    const json = await res.json();
    const ads = json?.data?.data || [];

    return ads.flatMap((ad) => {
      const path = `/ad-details/${ad?.slug}`;
      if (!ad?.slug) return [];
      return supportedLangs.map((lang) => ({
        url: buildUrlForLang(lang, path),
        lastModified: ad?.updated_at ? new Date(ad.updated_at).toISOString() : new Date().toISOString(),
