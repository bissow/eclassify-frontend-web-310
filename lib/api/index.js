"use client";
import Api from "@/lib/api/AxiosInterceptors";
export const GET_SETTINGS = "get-system-settings";
export const GET_SEO_SETTINGS = "seo-settings";
export const GET_SLIDER = "get-slider";
export const GET_CATEGORIES = "get-categories";
export const GET_PARENT_CATEGORIES = "get-parent-categories";
export const GET_ITEM_LIST = "get-item-list";
export const GET_PACKAGE = "get-package";
export const GET_BLOGS = "blogs";
export const GET_BLOG_TAGS = "blog-tags";
export const GET_FEATURED_SECTION = "get-featured-section";
export const GET_FAQ = "faq";
export const GET_COUNTRIES = "countries";
export const GET_STATES = "states";
export const GET_CITIES = "cities";
export const GET_AREA = "areas";
export const GET_LANGUAGE = "get-languages";
export const GET_CUSTOM_FIELDS = "get-customfields";
export const MANAGE_FAVOURITE = "manage-favourite";
export const GET_FAVOURITE_ITEMS = "get-favourite-item";
export const GET_MY_ITEMS = "my-items";
export const GET_LIMITS = "get-limits";
export const DELETE_ITEM = "delete-item";
export const UPDATE_ITEM_STATUS = "update-item-status";
export const CREATE_FEATURED_ITEM = "make-item-featured";
export const CONTACT_US = "contact-us";
export const UPDATE_LISTING = "update-item";
export const USER_SIGNUP = "user-signup";
export const USER_EXISTS = "user-exists";
export const UPDATE_PROFILE = "update-profile";
export const DELETE_USER = "delete-user";
export const GET_REPORT_REASONS = "get-report-reasons";
export const ADD_REPORT = "add-reports";
export const GET_NOTIFICATION_LIST = "get-notification-list";
export const CHAT_LIST = "chat-list";
export const CHAT_MESSAGES = "chat-messages";
export const SEND_MESSAGE = "send-message";
export const BLOCK_USER = "block-user";
export const UNBLOCK_USER = "unblock-user";
export const BLOCKED_USERS = "blocked-users";
export const ASSIGN_FREE_PACKAGE = "assign-free-package";
export const GET_PAYMENT_SETTINGS = "get-payment-settings";
export const PAYMENT_INTENT = "payment-intent";
export const PAYMENT_TRANSACTIONS = "payment-transactions";
export const MAKE_PAYMENT_TRANSACTION_FAIL = "make-payment-transaction-fail";
export const TIPS = "tips";
export const ITEM_OFFER = "item-offer";
export const ADD_ITEM = "add-item";
export const GET_SELLER = "get-seller";
export const ITEM_BUYER_LIST = "item-buyer-list";
export const AD_ITEM_REVIEW = "add-item-review";
export const GET_VERIFICATION_FIELDS = "verification-fields";
export const SEND_VERIFICATION_REQUEST = "send-verification-request";
export const GET_VERIFICATION_STATUS = "verification-request";
export const MY_REVIEWS = "my-review";
export const MY_PURCHASED_ITEMS = "my-purchased-items";
export const RENEW_ITEM = "renew-item";
export const ADD_REPORT_REVIEW = "add-review-report";
export const BANK_TRANSFER_UPDATE = "bank-transfer-update";
export const JOB_APPLY = "job-apply";
export const MY_JOB_APPLICATIONS = "my-job-applications";
export const GET_JOB_APPLICATIONS = "get-job-applications";
export const UPDATE_JOB_STATUS = "update-job-applications-status";
export const GET_OTP = "get-otp";
export const VERIFY_OTP = "verify-otp";
export const GET_LOCATION = "get-location";
export const GET_USER_INFO = "get-user-info";
export const LOGOUT = "logout";
export const SET_ITEM_TOTAL_CLICK = "set-item-total-click";
export const RESET_PASSWORD = "reset-password";
export const GET_CURRENCIES = "get-currencies";
export const GET_FOLLOWERS = "followers";
export const GET_FOLLOWING = "following";
export const FOLLOW_USER = "follow-user";
export const UNFOLLOW_USER = "unfollow-user";
export const GET_POPULAR_CATEGORIES = "get-popular-categories";
export const GET_CHAT_ADLIST = "item-offer-list";
export const DELETE_CHAT = "delete-chat";
export const DELETE_CHAT_MESSAGES = "delete-chat-messages";
export const GET_REFERRAL_CODE = "referral-code";
export const GET_REFER_POINTS_BALANCE = "refer-points-balance";
export const GET_USER_PURCHASED_PACKAGES = "get-user-purchased-packages";
export const GENERATE_META_DETAILS = "gemini/generate-meta";
export const GENERATE_DESCRIPTION = "gemini/generate-description";
export const GET_PAYMENT_RECEIPT = "get-payment-receipt";
export const GET_BANNER_ADS = "get-banner-ads";
export const GET_BLOG_CATEGORIES = "get-blog-categories";
export const GET_POPULAR_BLOGS = "get-popular-blogs";
export const SET_BLOG_FEEDBACK = "set-blog-feedback";
export const UPLOAD_MEDIA = "upload-media";
export const GET_MY_REEL = "get-my-reels";
export const GET_REELS = "get-reels";
export const MANAGE_REEL_LIKE = "manage-reel-like";
export const GET_LIKED_REELS = "get-liked-reels";
export const GET_CHAT_TEMPLATE_QUESTIONS = "chat-template-questions";
export const GET_STORES = "get-stores";
export const GET_STORE_DETAIL = "get-store-detail";
export const GET_STORE_SLUGS = "get-store-slugs";
export const SETUP_STORE = "setup-store";
export const GET_MY_STORE = "get-my-store";
export const TOGGLE_STORE_STATUS = "toggle-store-status";

// Offers & Campaigns (Public)
export const GET_OFFER_CAMPAIGNS = "offers/campaigns";
export const GET_OFFER_CAMPAIGN_DETAIL = "offers/campaign-detail";
export const GET_OFFER_PROMOTIONS = "offers/promotions";
export const GET_OFFER_PROMOTION_DETAIL = "offers/promotion-detail";
export const GET_OFFER_PROMOTION_ITEMS = "offers/promotion-items";
export const GET_FLASH_SALES = "offers/flash-sales";
export const GET_CLEARANCE_SALES = "offers/clearance-sales";
export const GET_DEALS_OF_THE_DAY = "offers/deals-of-the-day";
export const GET_SPOTLIGHT_ADS = "offers/spotlight-ads";

// Seller Promotions & Ad Boosts
export const SELLER_PROMOTIONS_AVAILABLE = "seller/promotions/available";
export const SELLER_PROMOTIONS_MY_ITEMS = "seller/promotions/my-items";
export const SELLER_PROMOTIONS_ANALYTICS = "seller/promotions/analytics";
export const SELLER_PROMOTIONS_HISTORY = "seller/promotions/history";
export const SELLER_PROMOTIONS_ADD_ITEM = "seller/promotions/add-item";
export const SELLER_PROMOTIONS_UPDATE_ITEM = "seller/promotions/update-item";
export const SELLER_PROMOTIONS_TOGGLE_STATUS = "seller/promotions/toggle-item-status";
export const SELLER_PROMOTIONS_DELETE_ITEM = "seller/promotions/delete-item";
export const SELLER_ITEM_PROMOTION_OPTIONS = "seller/items/promotion-options";
export const SELLER_ITEM_PROMOTE = "seller/items/promote";

// Seller QR Code Standee & Digital Catalog
export const SELLER_QR_ELIGIBILITY = "seller-qr/eligibility";
export const SELLER_MY_QR = "seller-qr/my-qr";
export const SELLER_QR_GENERATE_OR_UPDATE = "seller-qr/generate-or-update";
export const SELLER_QR_SETTINGS = "seller-qr/settings";
export const SELLER_QR_STORE = "seller-qr/store";
export const SELLER_QR_DOWNLOAD = "seller-qr/download";


// 1. SETTINGS API
export const settingsApi = {
  getSettings: ({ type } = {}) => {
    return Api.get(GET_SETTINGS, {
      params: { type },
    });
  },
};


// 2. SLIDER API
export const sliderApi = {
  getSlider: ({ country, state, city } = {}) => {
    return Api.get(GET_SLIDER, { params: { country, state, city } });
  },
};

// 3. CATEGORY API
export const categoryApi = {
  getCategory: ({ category_id, page, slug } = {}) => {
    return Api.get(GET_CATEGORIES, {
      params: { category_id, page, slug },
    });
  },
};
// 3. FEATURED CATEGORY API
export const popularCategoriesApi = {
  getPopularCategories: () => {
    return Api.get(GET_POPULAR_CATEGORIES);
  },
};
// 3. MY ITEMS API
export const getMyItemsApi = {
  getMyItems: ({ sort_by, page, status, id, category_id, slug } = {}) => {
    return Api.get(GET_MY_ITEMS, {
      params: { page, sort_by, status, id, category_id, slug },
    });
  },
};
export const getLimitsApi = {
  getLimits: ({ package_type } = {}) => {
    return Api.get(GET_LIMITS, {
      params: { package_type },
    });
  },
};
export const getSellerApi = {
  getSeller: ({ id, page } = {}) => {
    return Api.get(GET_SELLER, {
      params: { id, page },
    });
  },
};
export const getVerificationStatusApi = {
  getVerificationStatus: () => {
    return Api.get(GET_VERIFICATION_STATUS, {});
  },
};
export const getItemBuyerListApi = {
  getItemBuyerList: ({ item_id } = {}) => {
    return Api.get(ITEM_BUYER_LIST, {
      params: { item_id },
    });
  },
};
export const getVerificationFiledsApi = {
  getVerificationFileds: () => {
    return Api.get(GET_VERIFICATION_FIELDS, {});
  },
};

// get banner ads

// platform: app or web
// page: home, detail, listing
// layout: single, dual, single_side, dual_side, large  
// ad_type: only_banner, external_link, category, advertisement

export const getBannerAdsApi = {
  getBannerAds: ({ platform = 'web', page, layout, ad_type } = {}) => {
    return Api.get(GET_BANNER_ADS, {
      params: { platform, page, layout, ad_type },
    });
  },
};


export const getBlogCategoriesApi = {
  getBlogCategories: ({ page } = {}) => {
    return Api.get(GET_BLOG_CATEGORIES, {
      params: { page },
    });
  },
};

export const getPopularBlogsApi = {
  getPopularBlogs: ({ per_page = 4 } = {}) => {
    return Api.get(GET_POPULAR_BLOGS, {
      params: { per_page },
    });
  },
};

export const setBlogFeedbackApi = {
  setBlogFeedback: ({ blog_id, is_useful } = {}) => {
    const formData = new FormData();
    if (blog_id) formData.append("blog_id", blog_id);
    if (is_useful !== null && is_useful !== undefined) formData.append("is_useful", is_useful);
    return Api.post(SET_BLOG_FEEDBACK, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
  },
};


// 4. ITEM API
export const getItemListApi = {
  getItemsList: ({
    id,
    limit,
    page,
    category_id,
    user_id,
    min_price,
    max_price,
    latitude,
    longitude,
    radius,
    country,
    state,
    city,
    area_id,
    sort_by,
    custom_fields,

    posted_since,
    search,
    slug,
    category_slug,
    featured_section_slug,
    excluded_item_id
  } = {}) => {
    return Api.get(GET_ITEM_LIST, {
      params: {
        id,
        limit,
        page,
        category_id,
        user_id,
        min_price,
        max_price,
        latitude,
        longitude,
        radius,
        country,
        state,
        city,
        area_id,
        sort_by,
        custom_fields,

        posted_since,
        search,
        slug,
        category_slug,
        featured_section_slug,
        excluded_item_id
      },
    });
  },
};

// PACKAGE API

export const getPackageApi = {
  getPackage: ({ type, category_id } = {}) => {
    return Api.get(GET_PACKAGE, {
      params: {
        type,
        category_id,
      },
    });
  },
};

// BLOGS API
export const getBlogsApi = {
  getBlogs: ({ slug, category_id, tag, page, views, category_slug } = {}) => {
    return Api.get(GET_BLOGS, {
      params: {
        slug,
        category_id,
        tag,
        page,
        views,
        category_slug
      },
    });
  },
};
// BLOGS API
export const getBlogTagsApi = {
  getBlogs: ({ } = {}) => {
    return Api.get(GET_BLOG_TAGS, {
      params: {},
    });
  },
};
export const getMyReviewsApi = {
  getMyReviews: ({ page } = {}) => {
    return Api.get(MY_REVIEWS, {
      params: {
        page,
      },
    });
  },
};

export const getMyPurchasedItemsApi = {
  getMyPurchasedItems: ({ page, limit, search, is_reviewed } = {}) => {
    return Api.get(MY_PURCHASED_ITEMS, {
      params: {
        page,
        limit,
        search,
        is_reviewed,
      },
    });
  },
};

// 5. GET_FEATURED_SECTION
export const FeaturedSectionApi = {
  getFeaturedSections: ({
    city,
    state,
    country,
    slug,
    latitude,
    longitude,
    radius,
    area_id,
  } = {}) => {
    return Api.get(GET_FEATURED_SECTION, {
      params: {
        city,
        state,
        country,
        slug,
        latitude,
        longitude,
        radius,
        area_id,
      },
    });
  },
};
// FAQ API

export const getFaqApi = {
  getFaq: () => {
    return Api.get(GET_FAQ, {
      params: {},
    });
  },
};

// COUNTRY API

export const getCoutriesApi = {
  getCoutries: ({ search, page } = {}) => {
    return Api.get(GET_COUNTRIES, {
      params: {
        search,
        page,
      },
    });
  },
};

// STATES API

export const getStatesApi = {
  getStates: ({ country_id, search, page } = {}) => {
    return Api.get(GET_STATES, {
      params: {
        country_id,
        search,
        page,
      },
    });
  },
};

// CITIES API

export const getCitiesApi = {
  getCities: ({ state_id, search, page } = {}) => {
    return Api.get(GET_CITIES, {
      params: {
        state_id,
        search,
        page,
      },
    });
  },
};
export const getAreasApi = {
  getAreas: ({ city_id, search, page } = {}) => {
    return Api.get(GET_AREA, {
      params: {
        city_id,
        search,
        page,
      },
    });
  },
};

// language api

export const getLanguageApi = {
  getLanguage: ({ language_code, type } = {}) => {
    return Api.get(GET_LANGUAGE, {
      params: {
        language_code,
        type,
      },
    });
  },
};

export const userSignUpApi = {
  userSignup: ({
    password,
    name,
    email,
    mobile,
    fcm_id,
    firebase_id,
    type,
    profile,
    country_code,
    registration,
    region_code,
    is_login,
  } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (name) formData.append("name", name);
    if (email) formData.append("email", email);
    if (mobile) formData.append("mobile", mobile);
    if (fcm_id) formData.append("fcm_id", fcm_id);
    if (firebase_id) formData.append("firebase_id", firebase_id);
    if (type) formData.append("type", type);
    if (region_code) formData.append("region_code", region_code);

    // Assuming `profile` is a file object. If it's a URL or other type, handle accordingly.
    if (profile) {
      formData.append("profile", profile);
    }
    if (country_code) formData.append("country_code", country_code);
    if (registration) formData.append("registration", registration);
    if (is_login) formData.append("is_login", is_login);
    if (password) formData.append("password", password);

    return Api.post(USER_SIGNUP, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};
export const logoutApi = {
  logoutApi: ({ fcm_token } = {}) => {
    const formData = new FormData();
    if (fcm_token) formData.append("fcm_token", fcm_token);
    return Api.post(LOGOUT, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};

export const sendVerificationReqApi = {
  sendVerificationReq: ({
    verification_field,
    verification_field_files = [],
  } = {}) => {
    const formData = new FormData();

    // verification_field[<field_id>] — same key as verification_field_files,
    // so the API pairs a field's value and its upload. Files skipped here.
    Object.entries(verification_field || {}).forEach(([id, value]) => {
      if (value instanceof File) return;
      if (Array.isArray(value))
        value.forEach((v) => formData.append(`verification_field[${id}][]`, v));
      else if (value) formData.append(`verification_field[${id}]`, value);
    });

    verification_field_files.forEach(({ key, files }) => {
      const file = Array.isArray(files) ? files[0] : files;
      if (file) {
        formData.append(`verification_field_files[${key}]`, file);
      }
    });

    return Api.post(SEND_VERIFICATION_REQUEST, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};
export const updateProfileApi = {
  updateProfile: ({
    name,
    email,
    mobile,
    fcm_id,
    address,
    profile,
    notification,
    show_personal_details,
    country_code,
    region_code,
  } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (name) formData.append("name", name);
    if (email) formData.append("email", email);
    formData.append("mobile", mobile);
    if (fcm_id) formData.append("fcm_id", fcm_id);
    if (address) formData.append("address", address);
    if (country_code) formData.append("country_code", country_code);

    // Assuming `profile` is a file object. If it's a URL or other type, handle accordingly.
    if (profile) {
      formData.append("profile", profile);
    }
    formData.append("notification", notification);
    formData.append("show_personal_details", show_personal_details);
    formData.append("region_code", region_code);

    return Api.post(UPDATE_PROFILE, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};

// GET NOTIFICATION API

export const getNotificationList = {
  getNotification: ({ page } = {}) => {
    return Api.get(GET_NOTIFICATION_LIST, {
      params: {
        page: page,
      },
    });
  },
};

// ASSIGN FREE PACKAGE
export const assigFreePackageApi = {
  assignFreePackage: ({ package_id } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (package_id) formData.append("package_id", package_id);

    return Api.post(ASSIGN_FREE_PACKAGE, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};

export const addItemReviewApi = {
  addItemReview: ({ review, ratings, item_id } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (review) formData.append("review", review);
    if (ratings) formData.append("ratings", ratings);
    if (item_id) formData.append("item_id", item_id);

    return Api.post(AD_ITEM_REVIEW, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};
export const renewItemApi = {
  renewItem: ({ item_id, package_id } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (item_id) formData.append("item_id", item_id);
    if (package_id) formData.append("package_id", package_id);

    return Api.post(RENEW_ITEM, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};

// DELETE ITEM API

export const deleteItemApi = {
  deleteItem: ({ item_id, item_ids } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (item_id) formData.append("item_id", item_id);
    if (item_ids) formData.append("item_ids", item_ids);
    return Api.post(DELETE_ITEM, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};

export const chanegItemStatusApi = {
  changeItemStatus: ({ item_id, status, sold_to } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (item_id) formData.append("item_id", item_id);
    if (status) formData.append("status", status);
    if (sold_to) formData.append("sold_to", sold_to);

    return Api.post(UPDATE_ITEM_STATUS, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};
export const createFeaturedItemApi = {
  createFeaturedItem: ({ item_id, positions } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (item_id) formData.append("item_id", item_id);
    if (positions) formData.append("positions", positions);

    return Api.post(CREATE_FEATURED_ITEM, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};
// getPackageSettingsApi
export const getPaymentSettingsApi = {
  getPaymentSettings: () => {
    return Api.get(GET_PAYMENT_SETTINGS, {
      params: {},
    });
  },
};
// createPaymentIntentApi
export const createPaymentIntentApi = {
  createIntent: ({ package_id, payment_method, platform_type } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (package_id) formData.append("package_id", package_id);
    if (payment_method) formData.append("payment_method", payment_method);
    if (platform_type) formData.append("platform_type", platform_type);

    return Api.post(PAYMENT_INTENT, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};
// failPaymentTransactionApi
export const failPaymentTransactionApi = {
  failTransaction: ({ payment_transaction_id } = {}) => {
    return Api.post(MAKE_PAYMENT_TRANSACTION_FAIL, { payment_transaction_id });
  },
};
// deleteUserApi
export const deleteUserApi = {
  deleteUser: () => {
    return Api.delete(DELETE_USER, {
      params: {},
    });
  },
};
// paymentTransactionApi
export const paymentTransactionApi = {
  transaction: ({ page }) => {
    return Api.get(PAYMENT_TRANSACTIONS, {
      params: {
        page: page,
      },
    });
  },
};

// custom field api

export const getCustomFieldsApi = {
  getCustomFields: ({ filter, category_id } = {}) => {
    return Api.get(GET_CUSTOM_FIELDS, {
      params: {
        category_id,
        ...(filter !== undefined ? { filter } : {}),
      },
    });
  },
};

export const tipsApi = {
  tips: ({ } = {}) => {
    return Api.get(TIPS, {
      params: {},
    });
  },
};

export const itemOfferApi = {
  offer: ({ item_id, amount } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (item_id) formData.append("item_id", item_id);
    if (amount) formData.append("amount", amount);

    return Api.post(ITEM_OFFER, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};

export const chatListApi = {
  chatList: ({ type, page, item_id, search, item_offer_id } = {}) => {
    return Api.get(CHAT_LIST, {
      params: {
        type,
        page,
        item_id,
        item_offer_id,
        search: search || undefined,
      },
    });
  },
};

export const getMessagesApi = {
  chatMessages: ({ item_offer_id, page } = {}) => {
    return Api.get(CHAT_MESSAGES, {
      params: {
        item_offer_id,
        page,
      },
    });
  },
};
// add item api

export const addItemApi = {
  addItem: ({
    name,
    slug,
    description,
    category_id,
    price,
    contact,
    video_link,
    video_type,
    custom_fields,
    gallery_images = [],
    address,
    latitude,
    longitude,
    custom_field_files = [],
    area_id,
    country,
    state,
    city,
    min_salary,
    max_salary,
    translations,
    custom_field_translations,
    region_code,
    currency_id,
    image,
    seo_details,
    item_type
  } = {}) => {
    const formData = new FormData();
    // Append only if the value is defined and not an empty string

    if (name) formData.append("name", name);
    if (slug) formData.append("slug", slug);
    if (description) formData.append("description", description);
    if (category_id) formData.append("category_id", category_id);
    if (price) formData.append("price", price);
    if (contact) formData.append("contact", contact);
    if (video_link) formData.append("video_link", video_link);
    if (video_type) formData.append("video_type", video_type);

    if (custom_fields)
      formData.append("custom_fields", JSON.stringify(custom_fields));

    if (image) formData.append("image", image);

    if (gallery_images.length > 0) {
      gallery_images.forEach((gallery_image, index) => {
        formData.append(`gallery_images[${index}]`, gallery_image);
      });
    }
    if (address) formData.append("address", address);
    if (latitude) formData.append("latitude", latitude);
    if (longitude) formData.append("longitude", longitude);

    // Append custom field files
    custom_field_files.forEach(({ key, files }) => {
      if (Array.isArray(files)) {
        files.forEach((file, index) =>
          formData.append(`custom_field_files[${key}]`, file)
        );
      } else {
        formData.append(`custom_field_files[${key}]`, files);
      }
    });

    if (country) formData.append("country", country);
    if (state) formData.append("state", state);
    if (city) formData.append("city", city);
    if (area_id) formData.append("area_id", area_id);
    if (min_salary) formData.append("min_salary", min_salary);
    if (max_salary) formData.append("max_salary", max_salary);
    if (region_code) formData.append("region_code", region_code);
    if (currency_id) formData.append("currency_id", currency_id);
    if (item_type) formData.append("item_type", item_type);

    if (custom_field_translations)
      formData.append("custom_field_translations", custom_field_translations);

    if (translations) formData.append("translations", translations);
    if (seo_details) {
      seo_details.forEach(({ language_id, ...fields }) => {
        Object.entries(fields).forEach(([key, value]) => {
          formData.append(`seo_details[${language_id}][${key}]`, value);
        });
      });
    }



    return Api.post(ADD_ITEM, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};

// Edit item API
export const editItemApi = {
  editItem: ({
    id,
    name,
    slug,
    description,
    category_id,
    price,
    contact,
    video_link,
    video_type,
    custom_fields,
    gallery_images = [],
    address,
    latitude,
    longitude,
    custom_field_files = [],
    area_id,
    country,
    state,
    city,
    delete_item_image_id,
    min_salary,
    max_salary,
    translations,
    custom_field_translations,
    region_code,
    currency_id,
    seo_details,
    delete_product_video,
    // expiry_date,
  } = {}) => {
    const formData = new FormData();
    // Append only if the value is defined and not an empty string
    if (id) formData.append("id", id);
    if (name) formData.append("name", name);
    if (slug) formData.append("slug", slug);
    if (description) formData.append("description", description);
    if (category_id) formData.append("category_id", category_id);
    if (price) formData.append("price", price);
    if (delete_item_image_id && Array.isArray(delete_item_image_id)) {
      delete_item_image_id.forEach((id) => {
        formData.append("delete_item_image_id[]", id); // Note the [] in the key
      });
    }
    if (contact) formData.append("contact", contact);
    if (video_type) formData.append("video_type", video_type);
    if (video_link) formData.append("video_link", video_link);
    if (delete_product_video) formData.append("delete_product_video", 1);
    if (latitude) formData.append("latitude", latitude);
    if (longitude) formData.append("longitude", longitude);
    if (custom_fields)
      formData.append("custom_fields", JSON.stringify(custom_fields));
    if (address) formData.append("address", address);
    formData.append("contact", contact);
    if (country) formData.append("country", country);
    if (state) formData.append("state", state);
    // if (custom_field_files) formData.append("custom_field_files", custom_field_files)
    if (area_id) formData.append("area_id", area_id);
    if (city) formData.append("city", city);
    if (currency_id) formData.append("currency_id", currency_id);
    if (gallery_images.length > 0) {
      gallery_images.forEach((gallery_image, index) => {
        formData.append(`gallery_images[${index}]`, gallery_image);
      });
    }
    if (region_code) formData.append("region_code", region_code);

    formData.append("min_salary", min_salary);
    formData.append("max_salary", max_salary);
    // if (expiry_date) formData.append("expiry_date", expiry_date);

    custom_field_files.forEach(({ key, files }) => {
      if (Array.isArray(files)) {
        files.forEach((file, index) =>
          formData.append(`custom_field_files[${key}]`, file)
        );
      } else {
        formData.append(`custom_field_files[${key}]`, files);
      }
    });

    if (custom_field_translations)
      formData.append("custom_field_translations", custom_field_translations);

    if (translations) formData.append("translations", translations);
    seo_details?.forEach(({ language_id, ...fields }) => {
      Object.entries(fields).forEach(([key, value]) => {
        formData.append(`seo_details[${language_id}][${key}]`, value);
      });
    });
    return Api.post(UPDATE_LISTING, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};

export const sendMessageApi = {
  sendMessage: ({ item_offer_id, message, file, audio } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (item_offer_id) formData.append("item_offer_id", item_offer_id);
    if (message) formData.append("message", message);
    if (file) formData.append("file", file);
    if (audio) formData.append("audio", audio);

    return Api.post(SEND_MESSAGE, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};

// favorite API
export const manageFavouriteApi = {
  manageFavouriteApi: ({ item_id } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (item_id) formData.append("item_id", item_id);

    return Api.post(MANAGE_FAVOURITE, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};

export const getFavouriteApi = {
  getFavouriteApi: ({ page, per_page } = {}) => {
    return Api.get(GET_FAVOURITE_ITEMS, {
      params: {
        page,
        per_page
      },
    });
  },
};

export const getReportReasonsApi = {
  reportReasons: ({ } = {}) => {
    return Api.get(GET_REPORT_REASONS, {
      params: {},
    });
  },
};

export const addReportReasonApi = {
  addReport: ({ item_id, report_reason_id, other_message } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (item_id) formData.append("item_id", item_id);
    if (report_reason_id) formData.append("report_reason_id", report_reason_id);
    if (other_message) formData.append("other_message", other_message);

    return Api.post(ADD_REPORT, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};
export const addReportReviewApi = {
  addReportReview: ({ seller_review_id, report_reason } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (seller_review_id) formData.append("seller_review_id", seller_review_id);
    if (report_reason) formData.append("report_reason", report_reason);

    return Api.post(ADD_REPORT_REVIEW, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};

export const blockUserApi = {
  blockUser: ({ blocked_user_id } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (blocked_user_id) formData.append("blocked_user_id", blocked_user_id);

    return Api.post(BLOCK_USER, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};
export const unBlockUserApi = {
  unBlockUser: ({ blocked_user_id } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (blocked_user_id) formData.append("blocked_user_id", blocked_user_id);

    return Api.post(UNBLOCK_USER, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};

export const getBlockedUsers = {
  blockedUsers: ({ } = {}) => {
    return Api.get(BLOCKED_USERS, {
      params: {},
    });
  },
};

export const contactUsApi = {
  contactUs: ({ name, email, subject, message } = {}) => {
    const formData = new FormData();
    // Append only if the value is defined and not an empty string
    if (name) formData.append("name", name);
    if (email) formData.append("email", email);
    if (subject) formData.append("subject", subject);
    if (message) formData.append("message", message);
    return Api.post(CONTACT_US, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};
// get parent cate api
export const getParentCategoriesApi = {
  getPaymentCategories: ({ child_category_id, tree, slug } = {}) => {
    return Api.get(GET_PARENT_CATEGORIES, {
      params: {
        child_category_id,
        tree,
        slug,
      },
    });
  },
};

export const updateBankTransferApi = {
  updateBankTransfer: ({ payment_transaction_id, payment_receipt } = {}) => {
    const formData = new FormData();
    if (payment_transaction_id)
      formData.append("payment_transaction_id", payment_transaction_id);
    if (payment_receipt) formData.append("payment_receipt", payment_receipt);

    return Api.post(BANK_TRANSFER_UPDATE, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};

export const jobApplyApi = {
  jobApply: ({ item_id, full_name, email, mobile, resume } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (item_id) formData.append("item_id", item_id);
    if (full_name) formData.append("full_name", full_name);
    if (email) formData.append("email", email);
    if (mobile) formData.append("mobile", mobile);
    if (resume) formData.append("resume", resume);

    return Api.post(JOB_APPLY, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};

export const getMyJobApplicationsList = {
  getMyJobApplications: ({ page } = {}) => {
    return Api.get(MY_JOB_APPLICATIONS, {
      params: {
        page: page,
      },
    });
  },
};

export const getAdJobApplicationsApi = {
  getAdJobApplications: ({ page, item_id } = {}) => {
    return Api.get(GET_JOB_APPLICATIONS, {
      params: {
        page: page,
        item_id: item_id,
      },
    });
  },
};

export const updateJobStatusApi = {
  updateJobStatus: ({ job_id, status } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (job_id) formData.append("job_id", job_id);
    if (status) formData.append("status", status);

    return Api.post(UPDATE_JOB_STATUS, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};

export const verifyOtpApi = {
  verifyOtp: ({ number, otp, country_code, password } = {}) => {
    return Api.get(VERIFY_OTP, {
      params: {
        number: number,
        country_code: country_code,
        otp: otp,
        password: password
      },
    });
  },
};
export const getOtpApi = {
  getOtp: ({ number, country_code } = {}) => {
    return Api.get(GET_OTP, {
      params: {
        number: number,
        country_code: country_code
      },
    });
  },
};

export const getLocationApi = {
  getLocation: ({ lat, lng, lang, search, place_id, session_id } = {}) => {
    return Api.get(GET_LOCATION, {
      params: { lat, lng, lang, search, place_id, session_id },
    });
  },
};

export const getUserInfoApi = {
  getUserInfo: () => {
    return Api.get(GET_USER_INFO);
  },
};

export const getCurrenciesApi = {
  getCurrencies: ({ country } = {}) => {
    return Api.get(GET_CURRENCIES, {
      params: {
        country: country,
      },
    });
  },
};
export const getUserExistsApi = {
  getUserExists: ({ mobile, country_code, forgot_password, firebase_id } = {}) => {
    return Api.get(USER_EXISTS, {
      params: {
        mobile: mobile,
        country_code: country_code,
        forgot_password: forgot_password,
        firebase_id: firebase_id,
      },
    });
  },
};

export const setItemTotalClickApi = {
  setItemTotalClick: ({ item_id } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (item_id) formData.append("item_id", item_id);

    return Api.post(SET_ITEM_TOTAL_CLICK, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};


export const resetPasswordApi = {
  resetPassword: ({ number, country_code, new_password, token } = {}) => {
    const formData = new FormData();

    // Append only if the value is defined and not an empty string
    if (number) formData.append("number", number);
    if (country_code) formData.append("country_code", country_code);
    if (new_password) formData.append("new_password", new_password);

    // Prepare headers
    const headers = {
      "Content-Type": "multipart/form-data",
    };

    // Add authorization header if token is provided
    if (token) {
      headers.authorization = `Bearer ${token}`;
    }

    return Api.post(RESET_PASSWORD, formData, {
      headers
    });
  },
};


export const getSeoSettingsApi = {
  getSeoSettings: ({ page } = {}) => {
    return Api.get(GET_SEO_SETTINGS, {
      params: {
        page,
      },
    });
  },
};



export const getFollowersApi = {
  getFollowers: ({ user_id, search, page } = {}) => {
    return Api.get(GET_FOLLOWERS, {
      params: { user_id, search, page },
    });
  },
};

export const getFollowingApi = {
  getFollowing: ({ user_id, search, page } = {}) => {
    return Api.get(GET_FOLLOWING, {
      params: { user_id, search, page },
    });
  },
};


export const followUserApi = {
  followUser: ({
    user_id
  } = {}) => {
    const formData = new FormData();
    if (user_id) formData.append("user_id", user_id);
    return Api.post(FOLLOW_USER, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};

export const unfollowUserApi = {
  unfollowUser: ({
    user_id
  } = {}) => {
    const formData = new FormData();
    if (user_id) formData.append("user_id", user_id);
    return Api.post(UNFOLLOW_USER, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};

export const getChatAdListApi = {
  getChatAdList: ({ type, page, search } = {}) => {
    return Api.get(GET_CHAT_ADLIST, {
      params: {
        type,
        page,
        search
      },
    });
  },
};

export const deleteChatApi = {
  deleteChat: ({ item_offer_id = [] } = {}) => {
    const formData = new FormData();
    item_offer_id.forEach((id) => {
      formData.append("item_offer_id[]", id);
    });
    return Api.post(DELETE_CHAT, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};


export const deleteChatMessagesApi = {
  deleteChatMessages: ({ item_offer_id, message_ids = [] } = {}) => {
    const formData = new FormData();
    if (item_offer_id) formData.append("item_offer_id", item_offer_id);

    // Append each ID individually with [] to ensure backend receives it as an array
    message_ids.forEach((id) => {
      formData.append("message_ids[]", id);
    });

    return Api.post(DELETE_CHAT_MESSAGES, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};


export const getReferralCodeApi = {
  getReferralCode: () => {
    return Api.get(GET_REFERRAL_CODE);
  },
};


export const getReferPointsBalanceApi = {
  getReferPointsBalance: () => {
    return Api.get(GET_REFER_POINTS_BALANCE);
  },
};


export const getUserPurchasedPackagesApi = {
  getUserPurchasedPackages: ({ type } = {}) => {
    return Api.get(GET_USER_PURCHASED_PACKAGES, {
      params: {
        type
      }
    });
  },
};

export const generateMetaDetailsApi = {
  generateMetaDetail: ({
    title,
    price,
    currency_iso_code,
    language_id
  } = {}) => {
    const formData = new FormData();
    if (title) formData.append("title", title);
    if (price) formData.append("price", price);
    if (currency_iso_code) formData.append("currency_iso_code", currency_iso_code);
    if (language_id) formData.append("language_id", language_id);
    return Api.post(GENERATE_META_DETAILS, formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    });
  },
};

export const generateDescriptionApi = {
  generateDescription: ({ title, language_id, price, currency_iso_code } = {}) => {
    const formData = new FormData();
    if (title) formData.append("title", title);
    if (language_id) formData.append("language_id", language_id);
    if (price) formData.append("price", price);
    if (currency_iso_code) formData.append("currency_iso_code", currency_iso_code);
    return Api.post(GENERATE_DESCRIPTION, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
  },
};


export const uploadMediaApi = {
  uploadMedia: ({ item_id, video, thumbnail, product_video } = {}) => {
    const formData = new FormData();
    if (item_id) formData.append("item_id", item_id);
    if (video) formData.append("video", video);
    if (thumbnail) formData.append("thumbnail", thumbnail);
    if (product_video) formData.append("product_video", product_video);
    return Api.post(UPLOAD_MEDIA, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
  },
};

export const getPaymentReceiptApi = {
  getPaymentReceipt: ({ payment_transaction_id } = {}) => {
    return Api.get(GET_PAYMENT_RECEIPT, {
      params: { payment_transaction_id },
    });
  },
};

export const getMyReelsApi = {
  getMyReels: ({ item_id, per_page, reel_id, page } = {}) => {
    return Api.get(GET_MY_REEL, {
      params: { item_id, per_page, reel_id, page },
    });
  },
};

export const getReelsApi = {
  getReels: ({
    item_id,
    per_page,
    reel_id,
    page,
    following,
    country,
    state,
    city,
    area_id,
    latitude,
    longitude,
    radius,
  } = {}) => {
    return Api.get(GET_REELS, {
      params: {
        item_id,
        per_page,
        reel_id,
        page,
        following,
        country,
        state,
        city,
        area_id,
        latitude,
        longitude,
        radius,
      },
    });
  },
};

export const manageReelLikeApi = {
  manageReelLike: ({ reel_id } = {}) => {
    const formData = new FormData();
    if (reel_id) formData.append("reel_id", reel_id);
    return Api.post(MANAGE_REEL_LIKE, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
  },
};

export const getLikedReelsApi = {
  getLikedReels: ({ per_page, page } = {}) => {
    return Api.get(GET_LIKED_REELS, {
      params: { per_page, page },
    });
  },
};

export const getChatTemplateQuestionsApi = {
  getSettings: ({ item_id } = {}) => {
    return Api.get(GET_CHAT_TEMPLATE_QUESTIONS, {
      params: { item_id },
    });
  },
};

export const storesApi = {
  getStores: ({
    latitude,
    longitude,
    radius,
    country,
    state,
    city,
    area_id,
    search,
    is_verified,
    sort_by,
    page,
    limit,
  } = {}) => {
    return Api.get(GET_STORES, {
      params: {
        latitude,
        longitude,
        radius,
        country,
        state,
        city,
        area_id,
        search,
        is_verified,
        sort_by,
        page,
        limit,
      },
    });
  },

  getStoreDetail: ({
    slug,
    id,
    user_id,
    latitude,
    longitude,
    category_id,
    items_limit,
    items_page,
  } = {}) => {
    return Api.get(GET_STORE_DETAIL, {
      params: {
        slug,
        id,
        user_id,
        latitude,
        longitude,
        category_id,
        items_limit,
        items_page,
      },
    });
  },

  getStoreSlugs: () => {
    return Api.get(GET_STORE_SLUGS);
  },

  setupStore: (formData) => {
    return Api.post(SETUP_STORE, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
  },

  getMyStore: () => {
    return Api.get(GET_MY_STORE);
  },

  toggleStoreStatus: () => {
    return Api.post(TOGGLE_STORE_STATUS);
  },
};

// 47. OFFERS & CAMPAIGNS API (PUBLIC)
export const offersApi = {
  getCampaigns: ({ status = "active" } = {}) => {
    return Api.get(GET_OFFER_CAMPAIGNS, { params: { status } });
  },

  getCampaignDetail: ({ slug, id } = {}) => {
    return Api.get(GET_OFFER_CAMPAIGN_DETAIL, { params: { slug, id } });
  },

  getPromotions: ({ campaign_id, promotion_type } = {}) => {
    return Api.get(GET_OFFER_PROMOTIONS, { params: { campaign_id, promotion_type } });
  },

  getPromotionDetail: ({ slug, id } = {}) => {
    return Api.get(GET_OFFER_PROMOTION_DETAIL, { params: { slug, id } });
  },

  getPromotionItems: ({
    promotion_id,
    campaign_id,
    promotion_type,
    city,
    state,
    country,
    area_id,
    latitude,
    longitude,
    radius,
    sort_by,
    page,
    limit,
  } = {}) => {
    return Api.get(GET_OFFER_PROMOTION_ITEMS, {
      params: {
        promotion_id,
        campaign_id,
        promotion_type,
        city,
        state,
        country,
        area_id,
        latitude,
        longitude,
        radius,
        sort_by,
        page,
        limit,
      },
    });
  },

  getFlashSales: ({ city, state, country, area_id, latitude, longitude, radius, page, limit } = {}) => {
    return Api.get(GET_FLASH_SALES, {
      params: { city, state, country, area_id, latitude, longitude, radius, page, limit },
    });
  },

  getClearanceSales: ({ city, state, country, area_id, latitude, longitude, radius, sort_by, page, limit } = {}) => {
    return Api.get(GET_CLEARANCE_SALES, {
      params: { city, state, country, area_id, latitude, longitude, radius, sort_by, page, limit },
    });
  },

  getDealsOfTheDay: ({ city, state, country, area_id, latitude, longitude, radius, page, limit } = {}) => {
    return Api.get(GET_DEALS_OF_THE_DAY, {
      params: { city, state, country, area_id, latitude, longitude, radius, page, limit },
    });
  },

  getSpotlightAds: ({ city, state, country, area_id, latitude, longitude, radius, page, limit } = {}) => {
    return Api.get(GET_SPOTLIGHT_ADS, {
      params: { city, state, country, area_id, latitude, longitude, radius, page, limit },
    });
  },
};

// 48. SELLER PROMOTIONS API (AUTHENTICATED)
export const sellerPromotionsApi = {
  getAvailablePromotions: () => {
    return Api.get(SELLER_PROMOTIONS_AVAILABLE);
  },

  getMyPromotionItems: ({ status, page, limit } = {}) => {
    return Api.get(SELLER_PROMOTIONS_MY_ITEMS, { params: { status, page, limit } });
  },

  getPromotionsAnalytics: () => {
    return Api.get(SELLER_PROMOTIONS_ANALYTICS);
  },

  getPromotionsHistory: ({ filter_type, campaign_id, status, page, limit } = {}) => {
    return Api.get(SELLER_PROMOTIONS_HISTORY, {
      params: { filter_type, campaign_id, status, page, limit },
    });
  },

  addPromotionItem: ({
    promotion_id,
    item_id,
    discount_type,
    discount_value,
    promotional_price,
    stock_quantity,
    valid_until,
  } = {}) => {
    return Api.post(SELLER_PROMOTIONS_ADD_ITEM, {
      promotion_id,
      item_id,
      discount_type,
      discount_value,
      promotional_price,
      stock_quantity,
      valid_until,
    });
  },

  updatePromotionItem: ({ id, promotional_price, stock_quantity, valid_until } = {}) => {
    return Api.post(SELLER_PROMOTIONS_UPDATE_ITEM, {
      id,
      promotional_price,
      stock_quantity,
      valid_until,
    });
  },

  togglePromotionItemStatus: ({ id, status } = {}) => {
    return Api.post(SELLER_PROMOTIONS_TOGGLE_STATUS, { id, status });
  },

  deletePromotionItem: ({ id } = {}) => {
    return Api.post(SELLER_PROMOTIONS_DELETE_ITEM, { id });
  },

  getAdPromotionOptions: ({ item_id } = {}) => {
    return Api.get(SELLER_ITEM_PROMOTION_OPTIONS, { params: { item_id } });
  },

  promoteAd: ({ item_id, promotion_type, duration_days } = {}) => {
    return Api.post(SELLER_ITEM_PROMOTE, {
      item_id,
      promotion_type,
      duration_days,
    });
  },
};

// 49. SELLER QR CODE & DIGITAL CATALOG API
export const sellerQrApi = {
  // Check merchant entitlement to generate QR standee
  checkEligibility: () => {
    return Api.get(SELLER_QR_ELIGIBILITY);
  },

  // Fetch authenticated seller's QR code & scan analytics
  getMyQr: () => {
    return Api.get(SELLER_MY_QR);
  },

  // Generate or update standee branding (tagline, theme color, format, size)
  generateOrUpdate: (formData) => {
    const isFormData = typeof FormData !== "undefined" && formData instanceof FormData;
    return Api.post(SELLER_QR_GENERATE_OR_UPDATE, formData, {
      ...(isFormData ? { headers: { "Content-Type": "multipart/form-data" } } : {}),
    });
  },

  // Public system-wide QR settings (theme color, threshold, etc.)
  getSettings: () => {
    return Api.get(SELLER_QR_SETTINGS);
  },

  // Public: resolve store catalog by QR token or slug with location check
  getStoreByQr: ({
    identifier,
    latitude,
    longitude,
    city,
    state,
    category_id,
    search,
    sort_by,
    page = 1,
    limit = 16,
  } = {}) => {
    return Api.get(`${SELLER_QR_STORE}/${identifier}`, {
      params: {
        latitude,
        longitude,
        city,
        state,
        category_id,
        search,
        sort_by,
        page,
        limit,
      },
    });
  },

  // Helper to construct standee download URL
  getDownloadUrl: (token, { format = "pdf", size = "standee" } = {}) => {
    let apiBase = process.env.NEXT_PUBLIC_API_URL || "";
    if (!apiBase && typeof window !== "undefined") {
      apiBase = window.location.origin;
    }
    const cleanBase = apiBase.replace(/\/+$/, "");
    const endPoint = (process.env.NEXT_PUBLIC_END_POINT || "api/").replace(/^\/+|\/+$/g, "");
    return `${cleanBase}/${endPoint}/${SELLER_QR_DOWNLOAD}?token=${encodeURIComponent(token)}&format=${format}&size=${size}`;
  },
};
