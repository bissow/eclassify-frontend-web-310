...
export const getPackageApi = {
  getPackage: ({ type, category_id, listing_type, is_renew } = {}) => {
...
export const updateProfileApi = {
  updateProfile: ({
    ...
    // Append only passed fields, so callers can send a partial update.
    if (name) formData.append("name", name);
    if (email) formData.append("email", email);
    ...
    if (notification !== undefined) formData.append("notification", notification);
    if (show_personal_details !== undefined)
      formData.append("show_personal_details", show_personal_details);
    if (region_code !== undefined) formData.append("region_code", region_code);
...
export const editItemApi = {
  editItem: ({
    ...
    // TEMP DEBUG: hardcoded, remove before merge.
    // formData.append("expiry_date", "12-09-2026");
...
export const sendMessageApi = {
  sendMessage: ({ item_offer_id, message, file, audio, amount } = {}) => {
    const formData = new FormData();
...
export const getParentCategoriesApi = {
  getParentCategories: ({ child_category_id, tree, slug } = {}) => {
...
export const getUserPurchasedPackagesApi = {
  getUserPurchasedPackages: ({ type, category_id, item_type } = {}) => {
